...
def p_resolve_expected_market_price(params, substep, state_history, state):
    ...
    # interest rate / opportunity cost / time value of money
    ...
    # mean value from stochastic process of ETH price
    ...
    # Uniswap liquidity demand for RAI
    liquidity_demand = state['liquidity_demand']
    # mean value from stochastic process of liquidity
    liquidity_demand_mean = state['liquidity_demand_mean']
    ...
    if params['liquidity_demand_enabled'] == False:
        ...
        expected_market_price = p * (interest_rate +
                                     beta_1 * (eth_price_mean -
                                               eth_price * interest_rate)
                                     + beta_2 * (liquidity_demand_mean -
                                                 liquidity_demand * interest_rate))
...
def p_arbitrageur_model(params, substep, state_history, state):
    ...
    total_borrowed = aggregate_arbitrageur_cdp['drawn'] - \
        aggregate_arbitrageur_cdp['wiped'] - \
        aggregate_arbitrageur_cdp['u_bitten']
    total_deposited = aggregate_arbitrageur_cdp['locked'] - \
        aggregate_arbitrageur_cdp['freed'] - \
        aggregate_arbitrageur_cdp['v_bitten']

    if not total_borrowed >= 0:
        raise failure.NegativeBalanceException(total_borrowed)
    if not total_deposited >= 0:
        raise failure.NegativeBalanceException(total_deposited)
    ...
    if expensive_RAI_on_secondary_market:
        ...
        d_borrow = min(debt_ceiling - total_borrowed,
                       (_g1 - RAI_balance) / (1 - uniswap_fee))
        q_deposit = ((liquidation_ratio * redemption_price) /
                     eth_price) * (total_borrowed + d_borrow) - total_deposited
        z = (ETH_balance * d_borrow * (1 - uniswap_fee)) / \
            (RAI_balance + d_borrow * (1 - uniswap_fee))
        ...
        if is_cdp_above_liquidation_ratio(
            ...
            if q_deposit < 0:
                ...
                available_to_borrow = draw_to_liquidation_ratio(
                    aggregate_arbitrageur_cdp, eth_price, redemption_price, liquidation_ratio)
                if not available_to_borrow >= 0:
                    raise failure.ArbitrageConditionException(
                ...
                if d_borrow > available_to_borrow:
                    ...
                    if not delta_d_borrow >= 0:
                        raise failure.ArbitrageConditionException(
                            f'{delta_d_borrow=}')
                    q_deposit = ((liquidation_ratio * redemption_price) / eth_price) * \
                        (total_borrowed + delta_d_borrow) - total_deposited
                else:
                    q_deposit = 0
            ...
            if profit > 0:
                logging.debug(
                    f"{state['timestamp']} Performing arb. CDP -> UNI for profit {profit}")

                borrowed = cdps.at[aggregate_arbitrageur_cdp_index, "drawn"]
                deposited = cdps.at[aggregate_arbitrageur_cdp_index, "locked"]

                if not d_borrow >= 0:
                    raise failure.ArbitrageConditionException(f'{d_borrow=}')
                if not q_deposit >= 0:
                    raise failure.ArbitrageConditionException(f'{q_deposit=}')

                cdps.at[aggregate_arbitrageur_cdp_index,
                        "drawn"] = borrowed + d_borrow
                cdps.at[aggregate_arbitrageur_cdp_index,
                        "locked"] = deposited + q_deposit

                RAI_delta = d_borrow
                if not RAI_delta >= 0:
                    raise failure.ArbitrageConditionException(f'{RAI_delta=}')

                # Swap RAI for ETH
                _, ETH_delta = get_input_price(
                    d_borrow, RAI_balance, ETH_balance, uniswap_fee)
                if not ETH_delta < 0:
                    raise failure.ArbitrageConditionException(f'{ETH_delta=}')
                if not approx_eq(ETH_delta, -z, abs_tol=1e-5):
                    raise failure.ArbitrageConditionException(
                        f'{ETH_delta=} {-z=}')
        ...
        d_repay = (RAI_balance * z * (1 - uniswap_fee)) / \
            (ETH_balance + z * (1 - uniswap_fee))
        q_withdraw = total_deposited - \
            (liquidation_ratio * redemption_price /
             eth_price) * (total_borrowed - d_repay)
        ...
        if profit > 0:
            ...
            if not d_repay <= total_borrowed:
                raise failure.ArbitrageConditionException(
                    f"{d_repay=} {q_withdraw=} {_g2=} {RAI_balance=} {ETH_balance=} {total_borrowed=} {total_deposited=} {z=} {eth_price=} {redemption_price=} {market_price=}"
                )

            if not d_repay >= 0:
                raise failure.ArbitrageConditionException(f'{d_repay=}')
            if not q_withdraw >= 0:
                raise failure.ArbitrageConditionException(f'{q_withdraw=}')

            cdps.at[aggregate_arbitrageur_cdp_index,
                    "wiped"] = repayed + d_repay
            cdps.at[aggregate_arbitrageur_cdp_index,
                    "freed"] = withdrawn + q_withdraw

            # Deposit ETH, get RAI
            ETH_delta, _ = get_output_price(
                d_repay, ETH_balance, RAI_balance, uniswap_fee)
            if not ETH_delta > 0:
                raise failure.ArbitrageConditionException(f'{ETH_delta=}')
            if not approx_eq(ETH_delta, z, abs_tol=1e-5):
                raise failure.ArbitrageConditionException(f'{ETH_delta=} {z=}')
...
def validate_updated_cdp_state(cdps, previous_cdps, raise_on_assert=True):
    ...
    if not u_1 >= 0:
        raise failure.InvalidCDPStateException(f'{u_1}')
    if not u_2 >= 0:
        raise failure.InvalidCDPStateException(f'{u_2}')
    if not v_1 >= 0:
        raise failure.InvalidCDPStateException(f'{v_1}')
    if not v_2 >= 0:
        raise failure.InvalidCDPStateException(f'{v_2}')

    if not approx_greater_equal_zero(
        cdps["drawn"].sum() - cdps["wiped"].sum() - cdps["u_bitten"].sum(),
        abs_tol=1e-2,
    ):
        raise failure.InvalidCDPStateException(
            f'{cdps["drawn"].sum()=} {cdps["wiped"].sum()=} {cdps["u_bitten"].sum()=}')
    ...
    if not approx_greater_equal_zero(
        ...
        raise failure.InvalidCDPStateException(
            f'{cdps["locked"].sum()=} {cdps["freed"].sum()=} {cdps["v_bitten"].sum()=}')
